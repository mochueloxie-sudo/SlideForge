...
1. **`samples/themes/{design_mode}/tokens.css`**（若存在）— 手写精调  
2. 否则 **`utils/depth_tokens_from_tpl.js`** — 由 `DESIGN_TEMPLATES` 行生成完整 `--sf-*`（其余主题默认）  
3. 再否则最小 **`tplToCssVars`** 兜底  

**已提供 `samples/themes/{id}/tokens.css`（13/13 · matrix polish v4.2.7）**：全部手写精调，含完整 `--sf-*`（compare / stats-hero / process / visual-slot / page-bg-*）。换肤只改对应 `tokens.css`，深度 DOM 仍走 `samples/_core/layouts/`。

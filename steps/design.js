...
// ─── Content type → design_mode (shared with preview) ─────────────────────────
const dm = require('../utils/design_mode_infer');
...
function autoSelectDesignMode(scenes) {
  ...
  const contentType = dm.inferContentType(bodyText, scenes);
  const selected = dm.CONTENT_TYPE_MAP[contentType] || dm.DEFAULT_PRESET;

...
- **可选内容策展（Step 0.5）** — 当用户提供多份参考资料（PDF/飞书文档/网页）+ 内容框架时，Agent 在 Step 0 前做素材拼接与融合
...
- **9 个独立步骤**（含 1 个可选前置） — 运行完整流程或单独重跑任意步骤；所有中间产物持久化到磁盘
...
```
输入源（飞书 / .md / URL）+ 多份参考资料 + 内容框架
  │
  ├── [可选] Step 0.5 ── 内容策展（Agent 执行）
  │                     读所有参考资料 → 按框架拼接融合 → 输出完整 Markdown 内容稿
  │
  ▼
